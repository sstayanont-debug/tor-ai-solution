...
# 1) ลบ st.set_page_config ออกจากทุกหน้าย่อย (จะเรียกครั้งเดียวใน app.py แทน)
python3 - << 'PYEOF'
import glob
for path in glob.glob("pages/*.py"):
    with open(path, "r", encoding="utf-8") as f:
        lines = f.readlines()
    new_lines = [l for l in lines if "st.set_page_config(" not in l]
    if len(new_lines) != len(lines):
        with open(path, "w", encoding="utf-8") as f:
            f.writelines(new_lines)
        print(f"ลบ set_page_config ออกจาก {path}")
PYEOF

# 2) เขียน app.py ใหม่ทั้งไฟล์ (จัดการเมนูตามสิทธิ์)
cat > app.py << 'PYEOF'
"""
...
echo "เสร็จแล้ว: ลบ set_page_config ซ้ำออกจากทุกหน้า + เขียน app.py ใหม่"
